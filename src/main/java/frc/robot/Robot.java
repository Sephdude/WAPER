...
public class Robot extends TimedRobot 
{
  ...
  private void Drive()
  {
    if (driverInputAccepted)
    {
      ...
      if (yeetMode)
      {
        drive.arcadeDrive(-joystick.getLeftY() * 0.5f, joystick.getLeftX() * 0.5f, true);
      }
      else
      {
        drive.arcadeDrive(-joystick.getLeftY() * 0.75f, joystick.getLeftX() * 0.75f, true);
      }
  ...
  {
    if (joystick.getLeftTriggerAxis() > 0 && hasTarget == 1 && Math.abs(angleOffsetX) > 1)
    {
      double scale = 0.65f;
    ...
    if(joystick.getRightTriggerAxis() > 0.5f)
    {
  ...
  public void SearchX()
  {
      if (joystick.getRightTriggerAxis() > 0 && hasTarget == 1)
      {
          driverInputAccepted = false;
